...
describe("POST /bets ", () => {
  ...
  it("should put some bets on selected matches", async () => {
    const retrieveMatches = await request(server).get(
      "/championships/serie_a/h2h"
    );
...
describe("POST /bets/checkout/:betId", () => {
  ...
  describe("POST /bets/checkout/:betId", () => {
    beforeEach(() => (server = require("../api")));
    afterEach(() => server.close());
    it("should give error if user has got not enough money on his account", async () => {
      const login = await request(server)
        .post("/users/login")
        .send({ usernameOrEmail: "camillo", password: "123456ciao" });
      const token = login.header["x-auth-token"];
      ...
      expect(lastRes.status).toBe(402);
    });
